    ...
    <table id="main-frame" border="1">
        <div class="form-section">
            <div class="form-row">
                ...
                <label><div id="party-maintenance">Party Name </div>
                <select id="party-type" name="select" onchange="text1()"> 
                	<option value="0">select party-type</option>
                		...
                		try{
        					...
        					Statement statement=connection.createStatement();
        					 ResultSet rs = statement.executeQuery("select  * from party;");
    
        						 while (rs.next()) {
        		                        
        							 %>
        							 <option value=<%= rs.getString("party_id")%>>
            ...
            <div class="form-row">
                <label><div id="Transaction-type">Transaction-Type </div>
                 <select id="trn-type" name="select" onchange="text()"> 
                	<option value="0">select trn-type</option>
                		<%
                		...
                		try{
        					...
        					Connection connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/medicine", "root", "root");
        					Statement statement=connection.createStatement();
        					 ResultSet rs = statement.executeQuery("select  * from trantype;");
        					 
    
        						 while (rs.next()) {
        		                        
        							 %>
        							 <option value=<%= rs.getString("tt_id")%>>
        							 <%=rs.getString("tt_desc") %>
        							 </option>
            ...
            <div class="form-row">
                <label>Transaction-No. <input type="text" id="trn-no" placeholder="######" readonly> </label>
            </div>
        </div>
            <thead>
                <tr>
...
function save() {
    ...
    for(var i=1; i<tableObj.rows.length; i++) {
        ...
        for(var j=0; j<tableObj.rows[i].cells.length-2; j++) {
        	...
        	if(j<tableObj.rows[i].cells.length-3)
            	   sql = sql + ",'"; 
	...
	str+=sql;
	alert(sql);
    window.location.href='./pratice1.jsp?sql='+str;
...
function text(){
	...
	var text = document.getElementById("trn-type").value;
	document.getElementById("trn-no").value = text;
}
function text1(){
	
	var text = document.getElementById("party-type").value;
	document.getElementById("dl-no").value = text;
